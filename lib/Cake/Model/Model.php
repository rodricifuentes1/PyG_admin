...
// @codingStandardsIgnoreEnd
        ...
        foreach ($this->belongsTo as $parent => $assoc)
        {
            ...
            foreach ($assoc['counterCache'] as $field => $conditions)
            {
                ...
                if (isset($keys['old'][$foreignKey]) && $keys['old'][$foreignKey] != $keys[$foreignKey])
                {
                    ...
                    $Model->updateAll(
                            array ($field => $count), array ($Model->escapeField() => $keys['old'][$foreignKey])
                    );
                }
                ...
                $Model->updateAll(
                        array ($field => $count), array ($Model->escapeField() => $keys[$foreignKey])
                );
            }
        }
    }
    ...
    public function joinModel ($assoc, $keys = array ())
    {
        ...
        trigger_error(
                __d('cake_dev', 'Invalid join model settings in %s. The association parameter has the wrong type, expecting a string or array, but was passed type: %s', $this->alias, gettype($assoc)), E_USER_WARNING
        );
    }
