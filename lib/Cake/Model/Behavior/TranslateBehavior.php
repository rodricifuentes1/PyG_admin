...
class TranslateBehavior extends ModelBehavior
{
    ...
    public function setup (Model $Model, $config = array ())
    {
        ...
        {
            trigger_error(
                    __d('cake_dev', 'Datasource %s for TranslateBehavior of model %s is not connected', $Model->useDbConfig, $Model->alias), E_USER_ERROR
            );
            return false;
        }
    ...
    public function beforeFind (Model $Model, $query)
    {
        ...
        $fields = array_merge(
                $this->settings[$Model->alias], $this->runtime[$Model->alias]['fields']
        );
        $addFields = array ();
    ...
    public function afterValidate (Model $Model)
    {
        $Model->data[$Model->alias] = array_merge(
                $Model->data[$Model->alias], $this->runtime[$Model->alias]['beforeSave']
        );
        return true;
    }
        ...
        foreach ($fields as $key => $value)
        {
            ...
            else
            {
                ...
                foreach (array ('hasOne', 'hasMany', 'belongsTo', 'hasAndBelongsToMany') as $type)
                {
                    ...
                    {
                        trigger_error(
                                __d('cake_dev', 'Association %s is already bound to model %s', $association, $Model->alias), E_USER_ERROR
                        );
                        return false;
                    }
                }
