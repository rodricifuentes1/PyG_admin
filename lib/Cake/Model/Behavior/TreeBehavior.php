...
class TreeBehavior extends ModelBehavior
{
    ...
    protected function _recoverByParentId (Model $Model, $counter = 1, $parentId = null)
    {
        ...
        if ($parentId !== null)
        {
            ...
            {
                $Model->updateAll(
                        array ($this->settings[$Model->alias]['left'] => $counter), array ($Model->escapeField() => $parentId)
                );
                $counter++;
            }
            else
        ...
        {
            $Model->updateAll(
                    array ($this->settings[$Model->alias]['right'] => $counter), array ($Model->escapeField() => $parentId)
            );
            $counter++;
        }
