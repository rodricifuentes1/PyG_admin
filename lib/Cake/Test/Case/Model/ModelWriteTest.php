...
class ModelWriteTest extends BaseModelTest
{
    ...
    public function testAutoSaveUuid ()
    {
        ...
        $this->assertEquals(
                array ('id', 'title', 'count', 'created', 'updated'), array_keys($result['Uuid'])
        );
    ...
    public function testSaveFieldListResetsWhitelistOnFailedSave ()
    {
        ...
        $result = $model->save(
                array (), array ('fieldList' => array ('body'))
        );
        $this->assertFalse($result);
    ...
    {
        $this->loadFixtures(
                'CounterCacheUserNonstandardPrimaryKey', 'CounterCachePostNonstandardPrimaryKey'
        );
    ...
    {
        $this->loadFixtures(
                'User', 'Article', 'User', 'Comment', 'Tag', 'ArticlesTag', 'Attachment'
        );
        $TestModel = new User();
    ...
    public function testUpdateMultiple ()
    {
        ...
        $result = $TestModel->updateAll(
                array ('Comment.comment' => "'Updated today'"), array ('Comment.user_id' => 5)
        );
        $this->assertFalse(empty($result));
        $result = Hash::extract(
    ...
    public function testSaveAllDeepAssociated ()
    {
        ...
        unset(
                $result['Article']['created'], $result['Article']['updated'], $result['User']['created'], $result['User']['updated'], $result['Comment'][0]['created'], $result['Comment'][0]['updated']
        );
        $this->assertEquals($expected, $result);
    }
    ...
    public function testSaveAllAssociatedTransactionNoRollback ()
    {
        ...
        $this->getMock(
                'DboSource', array ('connect', 'rollback', 'describe', 'create', 'update', 'begin'), array (), 'MockTransactionAssociatedDboSource'
        );
    ...
    public function testSaveAllValidation ()
    {
        ...
        $this->assertEquals(self::date(), $result[3]['Post']['created']);
        unset(
                $result[0]['Post']['updated'], $result[1]['Post']['updated'], $result[3]['Post']['updated'], $result[3]['Post']['created']
        );
        $this->assertEquals($expected, $result);
        $this->assertEquals($errors, $TestModel->validationErrors);
    ...
    public function testSaveAllValidateFirst ()
    {
        ...
        $result = $model->find('all');
        $this->assertEquals(
                $result[0]['Article']['title'], 'Post with Author saveAlled from comment'
        );
    ...
    public function testSaveAssociated ()
    {
        ...
        $this->assertEquals(self::date(), $result[3]['Author']['updated']);
        unset(
                $result[3]['Post']['updated'], $result[3]['Post']['created'], $result[3]['Author']['updated'], $result[3]['Author']['created']
        );
        $this->assertEquals($expected, $result[3]);
        $this->assertEquals(4, count($result));
    ...
    public function testSaveAssociatedTransactionNoRollback ()
    {
        ...
        $this->getMock(
                'DboSource', array ('connect', 'rollback', 'describe', 'create', 'begin'), array (), 'MockAssociatedTransactionDboSource', false
        );
    ...
    public function testSaveAssociatedValidateFirst ()
    {
        ...
        $result = $model->find('all');
        $this->assertEquals(
                'Post with Author saveAlled from comment', $result[0]['Article']['title']
        );
        $this->assertEquals('Only new comment', $result[0]['Comment'][0]['comment']);
    }
