...
class TreeBehaviorNumberTest extends CakeTestCase
{
    ...
    public function testDetectEqualLftsRghts ()
    {
        ...
        $this->Tree->updateAll(array ($leftField => $this->Tree->escapeField($leftField) . ' -1'), array ($leftField . ' >' => $result[$modelClass][$leftField]));
        $this->Tree->updateAll(array ($rightField => $this->Tree->escapeField($rightField) . ' -1'), array ($rightField . ' >' => $result[$modelClass][$leftField]));
    ...
    public function testAddWithPreSpecifiedId ()
    {
        ...
        $expected = array_merge(
                array ($modelClass => array ('id' => 100, 'name' => 'testAddMiddle', $parentField => '2')), $result
        );
        $this->assertSame($expected, $result);
    ...
    public function testGenerateTreeListFormatting ()
    {
        ...
        $result = $this->Tree->generateTreeList(
                null, "{n}.$modelClass.id", array ('%s - %s', "{n}.$modelClass.id", "{n}.$modelClass.name")
        );
