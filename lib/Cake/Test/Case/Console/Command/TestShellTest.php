...
class TestShellTest extends CakeTestCase
{
    ...
    public function setUp ()
    {
        ...
        $this->Shell = $this->getMock(
                'TestTestShell', array ('in', 'out', 'hr', 'help', 'error', 'err', '_stop', 'initialize', '_run', 'clear'), array ($out, $out, $in)
        );
    ...
    public function testRunnerOptions ()
    {
        ...
        $this->Shell->expects($this->once())->method('_run')
                ->with(
                        array ('app' => false, 'plugin' => null, 'core' => true, 'output' => 'text', 'case' => 'Basics'), array ('--filter', 'myFilter', '--colors', '--verbose')
        );
        $this->Shell->main();
    }
