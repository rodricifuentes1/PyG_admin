...
class ViewTaskTest extends CakeTestCase
{
    ...
    public function setUp ()
    {
        ...
        $this->Task = $this->getMock('ViewTask', array ('in', 'err', 'createFile', '_stop'), array ($out, $out, $in)
        );
    ...
    public function testBakeView ()
    {
        ...
        $this->Task->expects($this->at(0))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'view.ctp', $this->stringContains('View Task Articles')
        );
    ...
    public function testBakeEdit ()
    {
        ...
        $this->Task->expects($this->at(0))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'edit.ctp', new PHPUnit_Framework_Constraint_IsAnything()
        );
    ...
    public function testBakeIndex ()
    {
        ...
        $this->Task->expects($this->at(0))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'index.ctp', $this->stringContains("\$viewTaskComment['Article']['title']")
        );
    ...
    public function testBakeActions ()
    {
        ...
        $this->Task->expects($this->at(0))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'view.ctp', $this->stringContains('View Task Comments')
        );
        $this->Task->expects($this->at(1))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'edit.ctp', $this->stringContains('Edit View Task Comment')
        );
        $this->Task->expects($this->at(2))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'index.ctp', $this->stringContains('ViewTaskComment')
        );
    ...
    public function testCustomAction ()
    {
        ...
        $this->Task->expects($this->once())->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'my_action.ctp', $this->anything()
        );
    ...
    public function testExecuteIntoAll ()
    {
        ...
        $this->Task->expects($this->at(0))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'index.ctp', $this->anything()
        );
        $this->Task->expects($this->at(1))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'add.ctp', $this->anything()
        );
    ...
    public function testExecuteWithActionParam ()
    {
        ...
        $this->Task->expects($this->once())->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'view.ctp', $this->anything()
        );
        $this->Task->execute();
    }
    ...
    public function testExecuteWithControllerAndAdminFlag ()
    {
        ...
        foreach ($views as $i => $view)
        {
            $this->Task->expects($this->at($i))->method('createFile')
                    ->with(
                            TMP . 'ViewTaskArticles' . DS . $view, $this->anything()
            );
        }
        $this->Task->execute();
        Configure::write('Routing', $_back);
    ...
    public function testExecuteInteractive ()
    {
        ...
        $this->Task->expects($this->at(4))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'view.ctp', $this->stringContains('ViewTaskComment')
        );

        $this->Task->expects($this->at(5))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'add.ctp', $this->stringContains('Add View Task Comment')
        );
    ...
    public function testExecuteWithAlternateTemplates ()
    {
        ...
        $this->Task->expects($this->once())->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'list.ctp', $this->stringContains('ViewTaskComment')
        );
        $this->Task->execute();
    }
    ...
    public function testExecuteInteractiveWithAdmin ()
    {
        ...
        $this->Task->expects($this->at(3))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'admin_index.ctp', $this->stringContains('ViewTaskComment')
        );

        $this->Task->expects($this->at(4))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'admin_view.ctp', $this->stringContains('ViewTaskComment')
        );

        $this->Task->expects($this->at(5))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'admin_add.ctp', $this->stringContains('Add View Task Comment')
        );

        $this->Task->expects($this->at(6))->method('createFile')
                ->with(
                        TMP . 'ViewTaskComments' . DS . 'admin_edit.ctp', $this->stringContains('Edit View Task Comment')
        );
