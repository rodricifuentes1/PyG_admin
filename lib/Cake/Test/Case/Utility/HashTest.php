...
class HashTest extends CakeTestCase
{
    ...
    public function testExtractUnevenKeys ()
    {
        ...
        $this->assertEquals(
                array ('test1', 'test2'), Hash::extract($data, 'Level1.Level2')
        );
        $this->assertEquals(
                array ('test3', 'test4'), Hash::extract($data, 'Level1.Level2bis')
        );
    ...
    public function testCombineWithFormatting ()
    {
        ...
        $result = Hash::combine(
                        $a, '{n}.User.id', array ('%1$s: %2$s', '{n}.User.Data.user', '{n}.User.Data.name'), '{n}.User.group_id'
        );
        $expected = array (
            1 => array (
        ...
        $result = Hash::combine(
                        $a, array ('%1$s: %2$d', '{n}.User.Data.user', '{n}.User.id'), '{n}.User.Data.name'
        );
        $expected = array (
        ...
        $result = Hash::combine(
                        $a, array ('%2$d: %1$s', '{n}.User.Data.user', '{n}.User.id'), '{n}.User.Data.name'
        );
        $expected = array (
            '2: mariano.iglesias' => 'Mariano Iglesias',
            '14: phpnut' => 'Larry E. Masters',
    ...
    public function testFormat ()
    {
        ...
        $result = Hash::format(
                        $data, array ('{n}.User.Data.user', '{n}.User.id'), '%s, %s'
        );
        $expected = array (
            'mariano.iglesias, 2',
            'phpnut, 14',
        ...
        $result = Hash::format(
                        $data, array ('{n}.User.Data.user', '{n}.User.id'), '%2$s, %1$s'
        );
        $expected = array (
            '2, mariano.iglesias',
            '14, phpnut',
