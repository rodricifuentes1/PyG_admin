    ...
    public function helpers ()
    {
        ...
        foreach ($plugins as $plugin)
        {
            ...
            $pluginHelpers = array_merge(
                    $pluginHelpers, App::objects('helper', App::pluginPath($plugin) . DS . 'views' . DS . 'helpers' . DS, false)
            );
        }
    ...
    protected function _findFiles ($extensions = '')
    {
        ...
        foreach ($this->_paths as $path)
        {
            ...
            $Iterator = new RegexIterator(
                    new RecursiveIteratorIterator(new RecursiveDirectoryIterator($path)), '/^.+\.(' . $extensions . ')$/i', RegexIterator::MATCH
            );
