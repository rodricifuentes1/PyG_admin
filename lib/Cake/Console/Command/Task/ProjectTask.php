...
class ProjectTask extends AppShell
{
    ...
    public function bake ($path, $skel = null, $skip = array ('empty'))
    {
        ...
        {
            $skel = $this->in(
                    __d('cake_console', "What is the path to the directory layout you wish to copy?"), null, CAKE . 'Console' . DS . 'Templates' . DS . 'skel'
            );
            if (!$skel)
            {
            ...
            else
            {
                ...
                {
                    $skel = $this->in(
                            __d('cake_console', 'Directory path does not exist please choose another:'), null, CAKE . 'Console' . DS . 'Templates' . DS . 'skel'
                    );
                }
            }
        }
