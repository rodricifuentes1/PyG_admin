...
class CakeTestFixture
{
    ...
    public function __construct ()
    {
        ...
        if (!empty($this->useDbConfig))
        {
            ...
            {
                $message = __d(
                        'cake_dev', 'Invalid datasource name "%s" for "%s" fixture. Fixture datasource names must begin with "test".', $connection, $this->name
                );
    ...
    public function init ()
    {
        ...
        {
            $import = array_merge(
                    array ('connection' => 'default', 'records' => false), is_array($this->import) ? $this->import : array ('model' => $this->import)
            );
    ...
    public function create ($db)
    {
        ...
        {
            $msg = __d(
                    'cake_dev', 'Fixture creation for "%s" failed "%s"', $this->table, $e->getMessage()
            );
            CakeLog::error($msg);
