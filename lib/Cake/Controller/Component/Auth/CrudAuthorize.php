...
class CrudAuthorize extends BaseAuthorize
{
    ...
    public function authorize ($user, CakeRequest $request)
    {
        ...
        {
            trigger_error(__d('cake_dev', 'CrudAuthorize::authorize() - Attempted access of un-mapped action "%1$s" in controller "%2$s"', $request->action, $request->controller
                    ), E_USER_WARNING
            );
            return false;
        }
        ...
        return $Acl->check(
                        $user, $this->action($request, ':controller'), $this->settings['actionMap'][$request->params['action']]
        );
    }
